...
table_for_summary_stats <- tableby(~BC_status + prevalent_status + incident_status + age_at_recruitment + sex + ethnicity_at_recruitment  + education_score + deprivation_index + alc_grouped + smoking_grouped  + mets_imaging_visit1 + bmi_recruitment + prev_status_diabetes + incident_status_diabetes + prev_status_hypertension + incident_status_hypertension + prev_status_cholesterol + incident_status_cholesterol + prev_status_IHD + incident_status_IHD + prev_status_NICM + incident_status_NICM + prev_status_HF + incident_status_HF + prev_status_stroke + incident_status_stroke + prev_status_pericard + incident_status_pericard + prev_status_myocard_inf + incident_status_myocard_inf, data = test_df,
                      numeric.stats=c("N", "median", "q1q3"), na.tableby(FALSE))
...
test_df <- cmr_df %>% dplyr::filter(f.eid %in% cmr_bc_rt_ids)
test_df <- cmr_df %>% dplyr::filter(f.eid %in% cmr_bc_but_no_mrt)

summary_cmr_measures <- tableby(~LVEDVI +  LV_mass2 + LVEF + LVGFI + GLS,
        numeric.stats=c( "median", "q1q3"), na.tableby(FALSE),
        data = test_df, )
...
## Cox Regression Models for CVD outcomes (BC vs no BC)
...
#### Store Cox regression results (BC vs no BC)
...
write.table(filtered_cox_combined_info, file = "BC_noBC_results_cox_regressions_table.tsv", sep = "\t", row.names = F)
saveRDS(filtered_cox_combined_info, file = "BC_noBC_results_cox_regressions_table.rds")
```


## Cox Regression Models for CVD outcomes (MRT vs no MRT in BC patients)


```{r cox_regressions_MRTvsNoMRT}
cox_models <- c()
# names(GLM_df)

### NICM
#### Model 1
test_df <- GLM_df %>% filter(prevalent_status ==1) %>%
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_NICM, incident_status_NICM, 
         age_at_recruitment, sex, recruitment_date, time) %>% filter(prev_status_NICM == 0)

formula = Surv(time, as.numeric(incident_status_NICM)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
tmp <- list(res.cox)
names(tmp) <- "NICM-Model1"
cox_models <- c(cox_models, tmp)
...
test_df <- GLM_df %>% filter(prevalent_status ==1) %>%
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_NICM, incident_status_NICM, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         prev_status_diabetes, prev_status_hypertension, prev_status_cholesterol) %>% 
  filter(prev_status_NICM == 0)

formula = Surv(time, as.numeric(incident_status_NICM)) ~ RT_status + 
  age_at_recruitment + sex +
# + bmi_recruitment + 
  smoking_status_at_recruitment + alc_intake_at_recruitment + 
  mets_imaging_visit1 + 
...
#### Model 1
test_df <- GLM_df %>% filter(prevalent_status ==1) %>%
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_IHD, incident_status_IHD, 
         age_at_recruitment, sex, recruitment_date, time) %>% filter(prev_status_IHD == 0)

formula = Surv(time, as.numeric(incident_status_IHD)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
tmp <- list(res.cox)
names(tmp) <- "IHD-Model1"
...
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_IHD, incident_status_IHD, 
         ...
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         prev_status_diabetes, prev_status_hypertension, prev_status_cholesterol) %>% 
  filter(prev_status_IHD == 0)

formula = Surv(time, as.numeric(incident_status_IHD)) ~ 
  RT_status + age_at_recruitment + sex + 
  # bmi_recruitment #+
  smoking_status_at_recruitment +
  alc_intake_at_recruitment +
  mets_imaging_visit1 +
  deprivation_index +
  ethnicity_at_recruitment +
...
### Stroke
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_stroke, incident_status_stroke, 
         age_at_recruitment, sex, recruitment_date, time) %>% filter(prev_status_stroke == 0)

formula = Surv(time, as.numeric(incident_status_stroke)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
tmp <- list(res.cox)
names(tmp) <- "Stroke-Model1"
...
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_stroke, incident_status_stroke, 
         ...
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         prev_status_diabetes, prev_status_hypertension, prev_status_cholesterol) %>% 
  filter(prev_status_stroke == 0)

formula = Surv(time, as.numeric(incident_status_stroke)) ~ RT_status + 
  age_at_recruitment + sex + 
  # bmi_recruitment + 
  smoking_status_at_recruitment + alc_intake_at_recruitment + 
  mets_imaging_visit1 + 
...
#### Model 1
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_pericard, incident_status_pericard, 
         age_at_recruitment, sex, recruitment_date, time) %>% filter(prev_status_pericard == 0)

formula = Surv(time, as.numeric(incident_status_pericard)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
tmp <- list(res.cox)
names(tmp) <- "Pericard-Model1"
...
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_pericard, incident_status_pericard, 
         ...
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         prev_status_diabetes, prev_status_hypertension, prev_status_cholesterol) %>% 
  filter(prev_status_pericard == 0)

formula = Surv(time, as.numeric(incident_status_pericard)) ~ RT_status + 
  age_at_recruitment + sex + 
  # bmi_recruitment +
  smoking_status_at_recruitment + alc_intake_at_recruitment + 
  mets_imaging_visit1 + 
...
#### Model 1
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_myocard_inf, 
         incident_status_myocard_inf, age_at_recruitment, sex, recruitment_date, time) %>%
  filter(prev_status_myocard_inf == 0)

formula = Surv(time, as.numeric(incident_status_myocard_inf)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
tmp <- list(res.cox)
names(tmp) <- "MYOINF-Model1"
...
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_myocard_inf, 
         incident_status_myocard_inf, bmi_recruitment,
...
formula = Surv(time, as.numeric(incident_status_myocard_inf)) ~ RT_status + 
  age_at_recruitment + sex + 
  # bmi_recruitment +
  smoking_status_at_recruitment + alc_intake_at_recruitment + 
  mets_imaging_visit1 + 
...
#### Model 1
test_df <- GLM_df %>% filter(prevalent_status ==1) %>% 
  select(f.eid, BC_status, RT_status, BC_diagnosis_date, prev_status_HF, incident_status_HF, 
         age_at_recruitment, sex, recruitment_date, time) %>% 
  filter(prev_status_HF == 0)

formula = Surv(time, as.numeric(incident_status_HF)) ~ RT_status + age_at_recruitment + sex
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
tmp <- list(res.cox)
names(tmp) <- "HF-Model1"
...
formula = Surv(time, as.numeric(incident_status_HF)) ~ RT_status + 
  age_at_recruitment + sex + 
  # bmi_recruitment + 
  smoking_status_at_recruitment + alc_intake_at_recruitment + 
  mets_imaging_visit1 + 
...
#### Store Cox regression results (MRT vs no MRT in BC patients)
...
# Specify the list of terms (variables) you want to include in the final merged table
# selected_terms <- c("BC_status1", "sex", "age_at_recruitment")  # Adjust this list as needed
selected_terms <- c("RT_status1")  # Adjust this list as needed
...
# Step 3: Extract information from each Cox model and add the model name
MRT_combined_cox_info <- data.frame()  # Initialize an empty data frame for the combined results

# Extract information from each named Cox model and add it to the combined table
for (model_name in names(cox_models)) {
  cox_model <- cox_models[[model_name]]
  cox_model_info <- extract_cox_model_info(cox_model, model_name)
  MRT_combined_cox_info <- bind_rows(MRT_combined_cox_info, cox_model_info)
}

# Filter the combined table to include only the selected terms
MRT_filtered_cox_combined_info <- MRT_combined_cox_info %>%
  filter(predictor %in% selected_terms)

# Print the filtered combined table with model names, selected predictor names, coefficients, p-values, and confidence intervals
MRT_filtered_cox_combined_info$p_value <- as.character(MRT_filtered_cox_combined_info$p_value)
MRT_filtered_cox_combined_info <- MRT_filtered_cox_combined_info %>% mutate_if(is.numeric, ~round(., 2))

View(MRT_filtered_cox_combined_info)

write.table(MRT_filtered_cox_combined_info, file = "MRT_noMRT_results_cox_regressions_table.tsv", sep = "\t",
            row.names = F)
# saveRDS(MRT_filtered_cox_combined_info, file = "MRT_noMRT_results_cox_regressions_table.rds")
```
