...
formula2 = incident_status_stroke ~ BC_status + 
  ...
  mets_imaging_visit1 + 
  deprivation_index + # education_score + ethnicity_at_recruitment +
  diabetes_status + cholesterol_status + hypertension_status

test_tbl <- GLM_df %>% filter(incident_status_stroke==0) 
model2 <- glm(formula2, data = test_tbl, family = "binomial")
summary(model2)


formula3 = incident_status_myocard_inf ~ BC_status + 
  age_at_recruitment + sex + ethnicity_at_recruitment +
