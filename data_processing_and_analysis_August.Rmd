...
t0 <- t0 %>% select(f.eid, cholesterol_status, date)

t1 <- t0 %>% group_by(f.eid, cholesterol_status) %>% summarise(minDate= min(date))

# tx <- cholesterol_tbl %>% group_by(f.eid, cholesterol_status) %>%
...
# t1<- t0 %>% group_by(f.eid, cholesterol_status) %>% summarise(minDate= min(date, na.rm=TRUE))
