...
table_one <- tableby(BC_status ~ ., data = GLM_df %>% select(-ethnicity_at_calib_visit, 
                                                             -ethnicity_at_calib_visit,
                                                             -ethnicity_at_imaging_visit1,
                                                             -alc_intake_at_recruitment,
                                                             -alc_intake_at_calib_visit,
                                                             -alc_intake_at_imaging_visit1,
                                                             -alc_intake_at_imaging_visit2,
                                                             -smoking_status_at_calib_visit,
                                                             -smoking_status_at_imaging_visit1,
                                                             -smoking_status_at_imaging_visit2,
                                                             -mets_imaging_visit2,
                                                             -time)) 
summary(table_one, title = "Gapminder Data")


table_four <- tableby(~BC_status + age_at_recruitment + sex + ethnicity_at_recruitment + deprivation_index, 
                      data = GLM_df,
                      numeric.stats=c("N", "median", "q1q3"),) 

sum_table <- summary(table_four, text = NULL)
sum_table <- as.data.frame(sum_table)
sum_table
write.table(sum_table, 'summary_stats_testDF.tsv', sep = "\t")

sink("summary_stats_testDF.txt")
( summary(table_four) )
sink()

library(broom)
glance(summary(table_four))
...
# demographic_table_BC <- demographic_table %>% dplyr::filter(f.eid %in% cmr_no_bc_ids)
# continuous <-select_if(demographic_table_BC, is.numeric)
# summary(continuous)

# demographic_table_BC <- demographic_table %>% dplyr::filter(f.eid %in% cmr_bc_ids)
# continuous <-select_if(demographic_table_BC, is.numeric)
# summary(continuous)

# demographic_table_BC <- demographic_table %>% dplyr::filter(f.eid %in% cmr_bc_but_no_mrt)
# continuous <-select_if(demographic_table_BC, is.numeric)
# summary(continuous)

demographic_table_BC <- demographic_table %>% dplyr::filter(f.eid %in% cmr_bc_rt_ids)
continuous <-select_if(demographic_table_BC, is.numeric)
summary(continuous)
...
tabyl((demographic_table$sex))
tabyl((demographic_table$ethnicity_at_recruitment))
tabyl((demographic_table$alc_intake_at_recruitment))
tabyl((demographic_table$smoking_status_at_recruitment))
...
df_list <- list(
                BC_tbl %>% select(-recruitment_date), 
...
# readRDS("table_for_regression_models.rds")
```
...
formula = incident_status_stroke ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)

test_df <- GLM_df %>% 
  select(f.eid, BC_status, prev_status_myocard_inf, incident_status_myocard_inf, 
         age_at_recruitment, sex) %>% filter(prev_status_myocard_inf==0)

formula = incident_status_myocard_inf ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)
# table_of_model
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, prev_status_pericard, incident_status_pericard, 
         age_at_recruitment, sex) %>% filter(prev_status_pericard==0)

formula = incident_status_pericard ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)

test_df <- GLM_df %>% 
  select(f.eid, BC_status, prev_status_HF, incident_status_HF, 
         age_at_recruitment, sex) %>% filter(prev_status_HF==0)

formula = incident_status_HF ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)

test_df <- GLM_df %>% 
  select(f.eid, BC_status, prev_status_NICM, incident_status_NICM, 
         age_at_recruitment, sex) %>% filter(prev_status_NICM==0)

formula = incident_status_NICM ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)
...
# IHD
test_df <- GLM_df %>% 
  select(f.eid, BC_status, prev_status_IHD, incident_status_IHD, 
         age_at_recruitment, sex) %>% filter(prev_status_IHD==0)

formula = incident_status_IHD ~ BC_status + age_at_recruitment + sex
model1 <- glm(formula, data = test_df, family = "binomial")
summary(model1)
table_of_model <- tidy(model1)
...
cmr_df <- full_join(cmr_measures_tbl, GLM_df, by = "f.eid")
...
## try Cox regression next
## then Fine Gray (compete risk) regression
# cmr_measures_tbl$LVGFI
...
summary(res.cox)
### ALCOHOL INTAKE AS ORDINAL OR FACTOR -> DIFFERENCE IN RESULTS ???
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_NICM, incident_status_NICM, 
         ...
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_NICM == 0)
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_IHD, incident_status_IHD, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_IHD == 0)
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_stroke, incident_status_stroke, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_stroke == 0)
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_pericard, incident_status_pericard, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_pericard == 0)
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_myocard_inf, incident_status_myocard_inf, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_myocard_inf == 0)
...
test_df <- GLM_df %>% 
  select(f.eid, BC_status, BC_diagnosis_date, prev_status_HF, incident_status_HF, 
         ...
         smoking_status_at_recruitment, ethnicity_at_recruitment,
         deprivation_index, alc_intake_at_recruitment, mets_imaging_visit1,
         diabetes_status, hypertension_status, cholesterol_status) %>% filter(prev_status_HF == 0)
