...
# Sample data
data <- mtcars  # Use the 'mtcars' dataset as an example

# Step 1: Create a list of linear regression models (you should have your own list)
lm_models_list <- list(
  model1 = lm(mpg ~ wt + hp, data = data),
  model2 = lm(mpg ~ wt + hp + qsec, data = data)
  # Add more linear regression models as needed
)

# Specify the list of predictors (variables) you want to include in the final merged table
selected_predictors <- c("wt", "hp")  # Adjust this list as needed
...
# Step 3: Extract information from each linear regression model and filter for selected predictors
extracted_info_list <- lapply(lm_models_list, extract_model_info)
filtered_info_list <- lapply(extracted_info_list, function(info) {
  filter(info, predictor %in% selected_predictors)
})

# Step 4: Combine the filtered information into a single data frame
combined_info <- do.call(rbind, filtered_info_list)

# Print the combined table with coefficients, confidence intervals, and p-values for selected predictors
print(combined_info)
...
res.cox  <- coxph(formula, data = test_df)
# summary(res.cox)
cox_models <- c(cox_models, list(res.cox))
